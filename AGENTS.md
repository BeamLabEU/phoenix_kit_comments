...
Attachment folder hook (`PhoenixKitComments.Attachments`): `config :phoenix_kit_comments, :attachments_parent_folder, {mod, fun}` (note the app is `:phoenix_kit_comments`, not `:phoenix_kit`), called ONCE per comment as `fun(:comment_attachment, actor_uuid, %{resource_type, resource_uuid})` (or the 2-arity `fun(:comment_attachment, actor_uuid)`) and answering `{:ok, folder_uuid}` or anything else for "media root". Placement goes through core's `Storage.attach_file_to_folder/2`: a homeless file is adopted, a file homed elsewhere (a checksum dedupe of the user's earlier upload) gets a `FolderLink`, never a move. The hook and the placement never raise or exit into the component: they run inside `consume_uploaded_entries/3`, where a crash loses a comment whose files are already stored. Non-uuid answers are dropped before any query.
